...
void MainWindow::on_pushButton_open_cmp_clicked()
{
    ...
    log_enable = json->Json_Get_Bool("config.json","日志记录使能");
    Excel_update();//更新表头数据
    QXlsx::Document diff_xlsx(Write_xlsx_name);//用于保存不同项
    //-------------------------------------------------------------
    // 设置单元格格式
    ...
    Format_cell.setFontColor (Qt::black);
    ...
    foreach (const QString& filename, same_list)//遍历
    {
        ...
        if((str_cmp->diff_A.length()+str_cmp->diff_B.length ()) !=0)
        {
            ...
            Format_cell.setHorizontalAlignment(QXlsx::Format::AlignLeft); //设置左对齐
            ...
            //写入描述信息和日期
            Format_cell.setHorizontalAlignment(QXlsx::Format::AlignHCenter); //设置居中
            ...
            diff_xlsx.write (write_row,COLUMN_HEAD_INDEX::Change_date,Change_date_str,Format_cell);
            diff_xlsx.write (write_row,COLUMN_HEAD_INDEX::Indx_cnt,write_row-2,Format_cell);
    ...
    write_row = 1;
    json->Json_Set_KeyValue("config.json","比较结果文件路径",Write_xlsx_name);
    //替换文件名中"/"
    Write_xlsx_name.replace("/","\\");
    ui->lineEdit_savepath->setText (Write_xlsx_name);
    Write_xlsx_name.clear();
