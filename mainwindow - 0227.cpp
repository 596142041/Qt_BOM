...
void MainWindow::on_pushButton_open_cmp_clicked()
{
    ...
    diff_xlsx.write (diff_row,1,"变更前Before\n"+File_Info.baseName (),format);

    File_Info.setFile(File_Name_New);
    format.setFontColor(Qt::red);   // 设置红色
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Model_Name_A,"变更后After\n"+File_Info.baseName (),format);
    ...
    format.setFontName("宋体");      // 设置字体
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Change_date,"变更日期",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Indx_cnt,"No",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Model_Name_B,"物料型号",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Factory_B,"厂家",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Description_B,"物料描述",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Point_B,"位号",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Quantity_B,"用量",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Model_Name_A,"物料型号",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Factory_A,"厂家",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Description_A,"物料描述",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Point_A,"位号",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Quantity_A,"用量",format);
    diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Change_type,"更改类型",format);
    diff_row++;
    //------------------------------------------------
    //直接使用list来查找不同
    //读取型号列
    ...
    foreach (const QString& filename, same_list)//遍历
    {
        ...
        if((str_cmp->diff_A.length()+str_cmp->diff_B.length ()) !=0)
        {
            ...
            diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Change_type,"",Format_cell);
            diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Change_date,Change_date_str,format);
            diff_xlsx.write (diff_row,COLUMN_HEAD_INDEX::Indx_cnt,diff_row-2,format);
