...
//        Factory_Cell.compare (dis_diffA_Factory_list->at (dis_cnt),Qt::CaseSensitive) == 0
        ...
        {
            //如果无差异项目,比对厂家是否相同
            if(Factory_Cell_A.compare (Factory_Cell,Qt::CaseSensitive) != 0)//厂家不同
            {
                //相同部分字体颜色
                Format_same.setFontColor (Qt::black);
                Format_same.setFontBold(false);       // 设置加粗
    ...
    //遍历dis_diffA_list
//    foreach (const QString& list, *dis_diffA_list)//遍历
//    {
//         qDebug()<<"dis_diffA_list:"<<list;
//    }
//    foreach (const QString& list, *dis_diffA_Factory_list)//遍历
//    {
//         qDebug()<<"dis_diffA_Factory_list:"<<list;
//    }
//    qDebug()<<"dis_start"<<dis_start<<"dis_diffA_Factory_list len"<<dis_diffA_Factory_list->length ()<<"dis_diffA_list len"<<dis_diffA_list->length ();
    //遍历B中不同项目
...
//        qDebug()<<"dis_cnt"<<dis_cnt<<"Read_cell_B"<<Read_cell_B<<"flag"<<flag;
        //------------------------------------------------------
        //旧版本BOM不同部分颜色
        Format_diff_B.setFontColor (QColor(0, 176, 240));
        ...
        else
        {
            ...
            //写入厂家
            //判断厂家
//            qDebug()<<"Factory_Cell:"<<Factory_Cell<<"dis_diffA_Factory_list"<<dis_diffA_Factory_list->at (dis_cnt);
            ...
            //diff_xlsx.write (dis_start+dis_cnt,COLUMN_HEAD_INDEX::Factory_A,Factory_Cell,Format_diff_A);//写入厂家

            //写入数量
            int Quantity_B = Read_cell_B.count (",")+1;
            Format_same.setHorizontalAlignment(QXlsx::Format::AlignHCenter); //设置左对齐
            diff_xlsx.write (dis_start+dis_cnt,COLUMN_HEAD_INDEX::Quantity_B,Quantity_B,Format_same);
            diff_xlsx.write (dis_start+dis_cnt,COLUMN_HEAD_INDEX::Quantity_A,Quantity_B,Format_same);
        }
        delete rich_diffB;
        delete rich_diffA;
        pros_cnt++;
